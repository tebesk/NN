...
for k in range(2000):
	...
	#img read(T)
	if  os.path.isfile("nntrn/"+Ansfiles[k]):#because Ansfiles are default images(not ans image)
		def_img = cv2.imread("nntrn/"+Ansfiles[k])
		def_img = cv2.cvtColor(def_img, cv2.COLOR_RGB2GRAY)			

		#we put all image in train array
		train_label_array.append(img.flatten().astype(np.float32))
		train_array.append(def_img.flatten().astype(np.float32))
	else:
		print Ansfiles[k]
		continue
